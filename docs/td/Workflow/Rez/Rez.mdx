...
Rez is an environment manager that allows us to launch DCCs in isolated contexts, include mandatory libraries only, define environment variables specific to this environment or define special cases for each team in the school.
...
:::tip
The team packages (5rn) are usually empty shells, it initially requires the render engine package used by the team and the aces config. But you can add specific tools or environments for their specific needs.
...
> 📝 **// Are we talking about rez repositories?** 📝
